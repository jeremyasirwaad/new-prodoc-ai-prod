...
const NewsCardHoverEffect = ({ items, className }) => {
  ...
  return (
    <div className="py-10">
      {items.map((item) => {
        ...
        if (item.attributes.featured) {
          return (
            <div
              key={item.id}
              className="relative group block p-2 h-full w-full mb-20"
            >
              <div
                id={item.id}
                onClick={() => router.push(`/news/${item.id}`)}
                className={cn(
                  "rounded-lg h-full w-full overflow-hidden relative z-20  cursor-pointer flex lg:flex-row flex-col",
                  className
                )}
              >
                <motion.img
      ...
      <div
        className={cn(
          "grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-16",
          className
        )}
      >
        {items.map((item) => {
            ...
            return (
              ...
              >
                <div
                  ...
                  className={cn(
                    "rounded-lg h-full w-full overflow-hidden relative z-20 cursor-pointer",
                    className
                  )}
                >
...
export default NewsCardHoverEffect;
