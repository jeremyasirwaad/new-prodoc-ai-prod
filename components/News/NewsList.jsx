...
import NewsCardHoverEffect from "./NewsCard";
      ...
      pages.push(
        <button
          ...
          className={`px-4 py-2 mx-1 ${
            i === currentPage ? "bg-gray-100" : "bg-gray-50"
          } rounded-lg`}
        >
          {i}
        </button>
  ...
  return (
    <div className=" min-h-[90vh]">
      {loading ? (
        ...
        <div>
          <NewsCardHoverEffect items={blogs} className="" />
          <div className="flex justify-center my-5">{renderPagination()}</div>
        </div>
      )}
    </div>
  );
